...
class AnalysisWork:
    ...
    def update_step(self, step_info: dict) -> None:
        ...
        if "status" in step_info and isinstance(step_info["status"], AudioStatus):
            step_info["status"] = "in_progress"

        # Ensure step_name exists
    ...
    def load_result_from_disk(self) -> bool:
        """
        Load the result field from disk if it exists and isn't already loaded.
        This is used for lazy loading to save memory.

        Returns:
            bool: True if result was loaded, False if already loaded or doesn't exist
        """
        if self.result is not None:
            # Already loaded
            return False

        file_path = config.DATA_DIR / "saved_jobs" / f"{self.id}.json"
        ...
        try:
            ...
            if "result" in job_dict and job_dict["result"]:
                self.result = TranscriptionResult(**job_dict["result"])
                print(f"Loaded result for job {self.id} from disk")
                return True
        except Exception as e:
            print(f"Error loading result for job {self.id}: {e}")
...
def load_analysis(job_id: str) -> AnalysisWork:
    """
...
def load_analysis_work_from_json(
    job_id: str, load_result: bool = False
) -> AnalysisWork:
    """
    Load an analysis job from a JSON file.
    ...
    Args:
        ...
        load_result: Whether to load the result field (can be memory intensive)
    ...
    try:
        ...
        # Restore result only if requested (lazy loading to save memory)
        if load_result and "result" in job_dict and job_dict["result"]:
            job.result = TranscriptionResult(**job_dict["result"])
...
def load_all_saved_jobs() -> Dict[str, AnalysisWork]:
    """
    Load all saved jobs from disk without loading result data to save memory.
    DEPRECATED: This loads all jobs into memory which can be memory intensive.
    Use get_saved_job_ids() and load jobs on-demand instead.
    ...
    for file_path in save_dir.glob("*.json"):
        ...
        # Load jobs without result data to save memory
        job = load_analysis_work_from_json(job_id, load_result=False)
        if job:
            jobs[job_id] = job

    print(f"Loaded {len(jobs)} saved jobs (results not loaded for memory efficiency)")
    return jobs


def get_saved_job_ids() -> List[str]:
    """
    Get a list of all saved job IDs without loading the job data.
    This is memory-efficient for getting an overview of available jobs.
...
def get_job_metadata(job_id: str) -> Optional[Dict[str, Any]]:
    """
    Get metadata for a job without loading the full job object.
    Useful for displaying job lists without consuming memory.

    Args:
        job_id: ID of the job

    Returns:
        Dict with basic job info or None if job doesn't exist
    """
    file_path = config.DATA_DIR / "saved_jobs" / f"{job_id}.json"
    ...
    try:
        ...
        return {
            ...
            "has_result": "result" in job_dict and job_dict["result"] is not None,
        }
    except Exception as e:
        print(f"Error reading metadata for job {job_id}: {e}")
        return None
