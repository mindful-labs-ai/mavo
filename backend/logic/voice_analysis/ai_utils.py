...
def get_seg_ts_with_diar_with_speaker_infer_short(
...
def get_seg_ts_with_diar_with_speaker_infer(trans_segs_with_ts, diarization_segments):
    ...
    if config.is_save_temp_files:
        save_path = (
            config.TEMP_DIR / f"tmp056_trans_segs_with_ts_with_diar_with_skips.json"
        )
        with open(save_path, "w", encoding="utf-8") as f:
            json.dump(trans_segs_with_ts_with_diar, f, ensure_ascii=False, indent=2)
        ...
        if seg["diar_label"] != current_diar:
            ...
            if current_seq:
                if len(current_seq) >= 7:
                    print(
                        f"long consecutive seq found. {len(current_seq)} segments -> leaving only few."
                    )
    ...
    save_path = (
        config.TEMP_DIR
        / f"tmp057_filtered_trans_segs_with_ts_with_diar_with_skips.json"
    )
    with open(save_path, "w", encoding="utf-8") as f:
...
"""
    ...
    text += "trans_segs_with_ts: " + json.dumps(
        trans_segs_with_ts_filt, ensure_ascii=False
    )
    text += "\n\n"
