...
import traceback
from backend.logic.models import AnalysisWork
...
async def identify_speakers_from_conversation(trans_segs_with_ts, unique_speakers):
    ...
    if not valid_speakers:
        ...
        return (
            [0]
            if 0 in unique_speakers
            else [unique_speakers[0] if unique_speakers else 0]
        ), [
            s
            for s in unique_speakers
            if s != 0 and s != (unique_speakers[0] if unique_speakers else 0)
        ]
    ...
    try:
        ...
        speaker_roles = sorted(
            analysis["speaker_roles"], key=lambda x: (x["role"], -x["confidence_norm"])
        )
        ...
        counselor_ids = [
            s["speaker_id"] for s in speaker_roles if s["role"] == "counselor"
        ]
        client_ids = [s["speaker_id"] for s in speaker_roles if s["role"] == "client"]
        uncertain_ids = [
            s["speaker_id"] for s in speaker_roles if s["role"] == "uncertain"
        ]
...
def save_transcription_result_json(
    audio_uuid: str, transcription_result, suffix: str = "transcript"
) -> None:
    """
    ...
    try:
        ...
        logger.error(
            f"Error saving final transcription result: {e}\n{traceback.format_exc()}"
        )
    ...
    for role, role_segments in role_groups.items():
        ...
        for cluster in clusters:
            if len(cluster) == 1:
                ...
                merged["word_segment_ids"] = [
                    seg["id"] for seg in cluster if "id" in seg
                ]
