...
from backend.logic.models import AnalysisWork, AudioStatus, analysis_jobs, TranscriptionResult, Segment, Speaker, save_analysis_work_to_json
import backend.config as config
from backend.logic.voice_analysis.ai_utils import assign_speaker_roles, assign_speaker_to_lines_with_gpt, get_openai_client, get_seg_ts_with_diar, get_seg_ts_with_diar_with_speaker_infer, get_seg_ts_with_diar_with_speaker_infer_short, get_seg_ts_with_diar_with_speaker_infer_wo_skip, get_seg_ts_with_diar_wo_ai, get_seg_ts_with_speaker_infer_wo_skip, improve_transcription_lines, improve_transcription_lines_parallel
from backend.logic.voice_analysis.audio_utils import process_audio_with_assembly_ai, process_audio_with_openai_api, split_audio_with_vad
from backend.logic.voice_analysis.common_utils import create_consecutive_segments, get_cleared_diarization_segments, save_analysis_work_json, save_transcription_result, save_transcription_result_json
...
async def diarize_audio_parallel(analysis_job, split_segments):
    ...
    try:
        ...
        logger.info(f"Starting parallel diarization for {len(split_segments)} segments of job {audio_uuid}")
        ...
        def fn_diarize_segment(segment_idx, segment):
            ...
            logger.info(f"Processing segment {segment_idx+1}/{len(split_segments)}: {segment_path}")
        ...
        if is_use_thread:
            with ThreadPoolExecutor(max_workers=5) as executor:
                for segment_idx, segment in enumerate(split_segments):
                    futures.append(executor.submit(fn_diarize_segment, segment_idx, segment))
        ...
        flattened_segments = [seg for segment_list in all_diarization_segments for seg in segment_list]
...
async def diarize_audio_whisperx(analysis_job, split_segments):
    ...
    diarize_segments = diarize_model(audio, num_speakers=None, max_speakers=MAX_SPEAKERS_IN_SPLIT * max_speaker_multiplier)
...
async def diarize_audio(file_path: str) -> List[Dict]:
    ...
    try:
        ...
        if not analysis_job:
            ...
            if job_id != audio_uuid:  # 다른 경우에만 중복 등록 (이 경우는 같지만 안전성을 위해)
...
def run_diarize_audio_whisperx(analysis_job, split_segments):
    return asyncio.run(diarize_audio_whisperx(analysis_job, split_segments))

def run_whisperx_transcription_and_diarization(analysis_job, split_segments):
    return asyncio.run(process_whisperx_transcription_and_diarization(analysis_job, split_segments))
...
async def do_vad_split(analysis_job: AnalysisWork) -> None:
    ...
    vad_mins = 2
    split_segments = split_audio_with_vad(analysis_job.file_path, analysis_job, target_duration=60*vad_mins)
...
async def process_analysis_job_gpt_diar_gpt2(analysis_job: AnalysisWork) -> None:
    ...
    try:
        ...
        trans_debug_path = transcript_dir / f"id[{audio_uuid}]_transcription_words_debug.json"
        ...
        trans_segs_with_ts = get_improved_lines_with_ts(trans_lines, result_transcription["segments"])
        # print("\ntrans_segs_with_ts", trans_segs_with_ts)
        print(f"len speaker: {len(trans_speakers)}, len trans_lines: {len(trans_lines)}, len transcription_words: {len(result_transcription['segments'])} len trans_segs_with_ts: {len(trans_segs_with_ts)}")
...
async def process_analysis_job_gpt_diar_gpt3(analysis_job: AnalysisWork, is_display: bool = False) -> None:
    ...
    try:
        ...
        if is_do_text_improv:
            ...
            # trans_lines = improve_transcription_lines(result_transcription["text"])
            trans_lines = improve_transcription_lines_parallel(transcription_words_splits)
            ...
            trans_segs_with_ts = transcription_words # this is already improved, segments.



        # logger.info(f"clear diarization result to have single speaker per segment")
        # diarization_segments_cleared = get_cleared_diarization_segments(diarization_segments)
        # if config.is_save_temp_files:
        ...
        # trans_segs_with_ts_with_diar = get_seg_ts_with_diar(trans_segs_with_ts, diarization_segments)
        # trans_segs_with_ts_with_diar = get_seg_ts_with_diar_wo_ai(trans_segs_with_ts, diarization_segments_cleared)
        # trans_segs_with_ts_with_diar = get_seg_ts_with_diar2(trans_segs_with_ts, diarization_segments)
        # trans_segs_with_ts_with_diar = get_seg_ts_with_diar_analytic(trans_segs_with_ts, diarization_segments)
        logger.info(f"infer speaker from transcription_segments and diarization_segments")
        # trans_segs_with_ts_with_diar = get_seg_ts_with_diar_with_speaker_infer_wo_skip(trans_segs_with_ts, diarization_segments_cleared)
        trans_segs_with_ts_with_diar = get_seg_ts_with_speaker_infer_wo_skip(trans_segs_with_ts)
        # trans_segs_with_ts_with_diar = get_seg_ts_with_diar_with_speaker_infer(trans_segs_with_ts, diarization_segments_cleared)
        # trans_segs_with_ts_with_diar = get_seg_ts_with_diar_with_speaker_infer_short(trans_segs_with_ts, diarization_segments_cleared)
        
        # print("\ntrans_segs_with_ts_with_diar", trans_segs_with_ts_with_diar)
        ...
        if is_display:
            ...
            try:
                ...
                for font_name in font_candidates:
                    if any(font_name.lower() in f.name.lower() for f in mpl.font_manager.fontManager.ttflist):
                ...
                if not font_found:
                    logger.warning("No suitable Korean font found. Some characters may not display correctly.")
                ...
                gs = fig.add_gridspec(5, 1, height_ratios=[4, 4, 4, 4, 1])  # 5 rows, last one for scrollbar
                ...
                try:
                    ...
                    for word_data in transcription_words:
                        if "text" in word_data and "start" in word_data and "end" in word_data:
                    ...
                    logger.warning(f"Could not add word markers to waveform: {word_error}")
                ...
                unique_speakers_in_result = list(set(segment.speaker for segment in segments))
                ...
                for segment in segments:
                    ...
                    color = speaker_colors[speaker % len(speaker_colors)] if speaker >= 0 else '#CCCCCC'
                    start = segment.start
                    end = segment.end
                    text = segment.text
                    ...
                    if speaker >= 0:
                        speaker_label = "상담사" if speaker == 0 else f"내담자 {speaker}"
                    else:
                        speaker_label = "미확인"
                ...
                has_undecided = any(segment.speaker == "undecided" for segment in segments)
                ...
                try:
                    ...
                    if "diarizing" in analysis_job.steps:
                        ...
                        if hasattr(diar_step, "get") and diar_step.get("diarization_segments"):
                            diarization_segments = diar_step["diarization_segments"]
                            logger.info(f"Found {len(diarization_segments)} diarization segments to visualize")
                    
                    # Fallback to frontend-friendly diarization data
                    if not diarization_segments and analysis_job.step_results.get("diarization"):
                        diarization_segments = analysis_job.step_results["diarization"].get("segments", [])
                        logger.info(f"Using {len(diarization_segments)} segments from step_results")
                    
                    # See if we can load from file as last resort
                    if not diarization_segments:
                        audio_uuid = analysis_job.audio_uuid
                        diar_path = config.UPLOADS_DIR / str(audio_uuid) / f"id[{audio_uuid}]_ml_diarized.json"
                        if os.path.exists(diar_path):
                            with open(diar_path, 'r') as f:
                                diar_data = json.load(f)
                                if 'diarization_segments' in diar_data:
                                    diarization_segments = diar_data['diarization_segments']
                                    logger.info(f"Loaded {len(diarization_segments)} segments from file")


                    unique_speakers_in_diar = list(set(segment.get("speaker", 0) for segment in diarization_segments))
                    ...
                    if diarization_segments:
                        ...
                        row_height = 0.15
                        row_spacing = 0.2
                        max_rows = len(unique_speakers_in_diar)  # Use all unique speakers instead of fixed max
                        speaker_rows = {}
                        ...
                        for segment in diarization_segments:
                            ...
                            if speaker in speaker_rows:
                                ...
                                ax3.barh(row_pos, end-start, left=start, height=row_height, 
                                        color=color, alpha=0.7, edgecolor='black', linewidth=0.5)
                                
                                # Add speaker label in the middle of segment if long enough
                                if end - start > 2.0:
                                    ax3.text(start + (end-start)/2, row_pos, 
                                            f"화자 {speaker}", 
                                            ha='center', va='center', 
                        ...
                        for speaker in sorted(speaker_rows.keys()):
                            color = diar_colors[speaker % len(diar_colors)]
                            legend_elements.append(Patch(facecolor=color, label=f"화자 {speaker}"))
                        ...
                        try:
                            ...
                            for word_data in word_timestamps:
                                ...
                                ax3.axvline(x=start, color='green', linestyle='-', alpha=0.4, linewidth=0.7)
                                
                                # Add vertical line at word END (red)
                                ax3.axvline(x=end, color='red', linestyle=':', alpha=0.4, linewidth=0.7)
                                
                                # Add word text at the word start position
                                ax3.text(start, y_pos, word, fontsize=7, color='black', rotation=90, 
                                ...
                                ax3.plot([start, end], [span_height, span_height], 
                                        color='blue', alpha=0.3, linewidth=2)
                        except Exception as word_marker_error:
                            logger.warning(f"Error adding word markers to diarization: {word_marker_error}")
                            # Continue even if word marking fails
                    else:
                        ax3.text(view_end/2, 0, "No diarization data available", 
                                ha='center', va='center', fontsize=12)
                except Exception as diar_error:
                    logger.error(f"Error plotting diarization data: {diar_error}")
                    ax3.text(view_end/2, 0, "Error loading diarization data", 
                            ha='center', va='center', fontsize=12)
                ...
                try:
                    ...
                    if diarization_segments_cleared:
                        unique_speakers_in_cleared = list(set(segment.get("speaker", 0) for segment in diarization_segments_cleared))
                        ...
                        if diarization_segments_cleared:
                            ...
                            row_height = 0.15
                            row_spacing = 0.2
                            max_rows = len(unique_speakers_in_cleared)  # Use all unique speakers
                            speaker_rows = {}
                            ...
                            for segment in diarization_segments_cleared:
                                ...
                                if speaker in speaker_rows:
                                    ...
                                    # Draw segment as horizontal bar
                                    ax4.barh(row_pos, end-start, left=start, height=row_height, 
                                            color=color, alpha=0.7, edgecolor='black', linewidth=0.5)
                                    
                                    # Add speaker label in the middle of segment if long enough
                                    if end - start > 2.0:
                                        ax4.text(start + (end-start)/2, row_pos, 
                                                f"화자 {speaker}", 
                                                ha='center', va='center', 
                                                fontsize=8, bbox=dict(boxstyle='round', facecolor='white', alpha=0.7))
                            ...
                            ax4.text(view_end/2, 0, "No cleared diarization data available", 
                                    ha='center', va='center', fontsize=12)
                    else:
                        ax4.text(view_end/2, 0, "No cleared diarization data available", 
                                ha='center', va='center', fontsize=12)
                except Exception as diar_error:
                    logger.error(f"Error plotting cleared diarization data: {diar_error}")
                    ax4.text(view_end/2, 0, "Error loading cleared diarization data", 
                           ha='center', va='center', fontsize=12)
                ...
                try:
                    ...
                    for word_data in word_timestamps:
                        ...
                        # Add vertical line at word start (green)
                        ax1.axvline(x=start, color='green', linestyle='-', alpha=0.4, linewidth=0.5)
                        
                        # Add vertical line at word end (red)
                        ax1.axvline(x=end, color='red', linestyle=':', alpha=0.4, linewidth=0.5)
                        ...
                        ax1.plot([start, end], [y_min + span_height*0.5, y_min + span_height*0.5], 
                                color='blue', alpha=0.3, linewidth=2)
                        
                        # Add word label centered on the span
                        if end - start > 0.3:  # Only add labels for longer words
                            ax1.text(start + (end-start)/2, y_min + span_height, word, 
                                    fontsize=8, ha='center', va='bottom', 
                                    bbox=dict(boxstyle='round', facecolor='white', alpha=0.7))
                except Exception as word_marker_error:
                    logger.warning(f"Error adding word start/stop markers: {word_marker_error}")
                    # Continue without word markers
...
async def process_analysis_job_gpt_diar_mlpyan2(analysis_job: AnalysisWork, is_display: bool = False) -> None:
    ...
    try:
        ...
        analysis_job.update_status(AudioStatus.TRANSCRIBING)
        transcription_task = asyncio.create_task(
            # asyncio.to_thread(run_whisperx_transcription_and_diarization, analysis_job, analysis_job.split_segments), 
            asyncio.to_thread(run_process_transcription, analysis_job, analysis_job.split_segments), 
            name="transcription"
        )
        tasks.append(transcription_task)
        ...
        analysis_job.update_status(AudioStatus.DIARIZING)
        diarization_task = asyncio.create_task(
            asyncio.to_thread(run_diarize_audio_whisperx, analysis_job, analysis_job.split_segments), 
            # asyncio.to_thread(run_diarize_audio_parallel, analysis_job, analysis_job.split_segments), 
            name="diarization"
        )
        tasks.append(diarization_task)
        ...
        # results = [None]
        # results[0] = process_whisperx_transcription_and_diarization(analysis_job, analysis_job.split_segments)
        
        # Handle results
        result_transcription = results[0] # diarized transcript result!
        # diarization_segments = results[0]['segments']
        diarization_segments = results[1]
        ...
        diar_word_align_result = whisperx.assign_word_speakers(diarization_segments, trans_result)
        ...
        frontend_diarization = {
            ...
            "total_duration": max([seg["end"] for seg in diar_segs]) if diar_segs else 0,
            "num_speakers": len(set(seg["speaker"] for seg in diar_segs if "speaker" in seg))
        }
        ...
        save_path = config.UPLOADS_DIR / audio_uuid / f"id[{audio_uuid}]_transcription_with_role.json"
        with open(save_path, "w", encoding="utf-8") as f:
        ...
        save_path = config.UPLOADS_DIR / audio_uuid / f"id[{audio_uuid}]_consecutive_segments.json"
        with open(save_path, "w", encoding="utf-8") as f:
        ...
        continuous_segments = [seg for seg in continuous_segments if seg['speaker_role'] != -1]
        ...
        speakers = [
            Speaker(
            id=i,
            role="counselor" if i == 0 else f"client{i}"
            )
        ...
        ## save result file
        save_path = config.UPLOADS_DIR / audio_uuid / f"id[{audio_uuid}]_transcription_result.json"
        with open(save_path, "w", encoding="utf-8") as f:
...
async def process_whisperx_transcription_and_diarization(analysis_job: AnalysisWork, split_segments: List[Dict]) -> Dict:
    ...
    def fn_whisperx_transcription_and_diarization(idx_split, split_segment):
        try:
            ...
            logger.info(f"Processing split {idx_split+1}/{total_splits}: {segment_path} in process_whisperx_transcription_and_diarization")
            ...
            print("Assigning speakers to segments...")
            # diar_word_align_result = whisperx.assign_word_speakers(diarize_segments, aligned_result, fill_nearest=True)
            diar_word_align_result = whisperx.assign_word_speakers(diarize_segments, aligned_result)
            ...
            for segment in diar_segs:
                ...
                if segment['speaker'] != -1:
                    segment['speaker'] = segment['speaker'] + idx_split * MAX_SPEAKERS_IN_SPLIT
            ...
            logger.exception(f"Split {idx_split} failed, error: {e} with traceback: {traceback.format_exc()}")
            raise
    ...
    if is_use_thread:
        with ThreadPoolExecutor(max_workers=3) as executor:
            for idx_split, split_segment in enumerate(split_segments):
                futures.append(executor.submit(fn_whisperx_transcription_and_diarization, idx_split, split_segment))
