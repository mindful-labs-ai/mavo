...
async def process_analysis_job_gpt_diar_mlpyan2(
    analysis_job: AnalysisWork, is_display: bool = False
) -> None:
    """Process the analysis job using GPT for transcription and MLP-YAN for diarization."""
    try:
        ...
        diarization_task = asyncio.create_task(
            asyncio.to_thread(
                run_diarize_audio_whisperx, analysis_job, analysis_job.split_segments
            ),
            # asyncio.to_thread(run_diarize_audio_parallel, analysis_job, analysis_job.split_segments),
            name="diarization",
        )
        ...
        # Handle results
        result_transcription = results[0] or {
            "segments": []
        }  # diarized transcript result!
        # diarization_segments = results[0]['segments']
        diarization_segments = results[1] if results[1] else []

        print("Async jobs done")
        print("len result_transcription", len(result_transcription["segments"]))
        # print("len diarization_segments", diarization_segments)
